...
public abstract class ApiMessageClient {
  ...
  /**
   * HTTP 요청을 받아 적절한 서비스로 라우팅하여 처리합니다. 요청 경로를 분석하여 해당하는 서비스의 메시지 큐로 RabbitMQ를 통해 전송하며, 비동기적으로 처리된
   * 결과를 응답으로 반환합니다.
   *
   * <p>주요 처리 과정: 1. 요청 경로를 분석하여 적절한 라우팅 키 결정 (/api/auth -> auth-service 등) 2. HTTP 요청 데이터를 메시지 형태로
   * 변환 3. RabbitMQ를 통해 해당 서비스로 메시지 전송 4. 비동기 응답을 대기하고 결과 반환
   *
   * @param <T> {@code BaseApiResponse} 내에 캡슐화된 응답 본문의 타입
   * @param exchange HTTP 요청과 응답 정보를 포함하는 {@code ServerWebExchange}
   * @return 비동기 처리가 완료되면 응답 데이터가 포함된 {@code BaseApiResponse}를 담고 있는 {@code ResponseEntity}를 방출하는
   *     {@code Mono}
   */
