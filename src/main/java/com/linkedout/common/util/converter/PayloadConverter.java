...
public class PayloadConverter {
	...
	/**
	 * 페이로드 객체를 지정된 제네릭 타입으로 안전하게 변환 (TypeReference 사용)
	 *
	 * @param payload       변환할 페이로드 객체
	 * @param typeReference 변환 대상 타입 참조
	 * @param <T>           변환 대상 타입
	 * @return 변환된 객체
	 */
	public <T> T convert(Object payload, TypeReference<T> typeReference) {
		if (payload == null) {
			return null;
		}

		return objectMapper.convertValue(payload, typeReference);
	}
}
